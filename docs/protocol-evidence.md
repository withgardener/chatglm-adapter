...
| 图片上传链路（多模态） | A | 2026-09-11 HAR：`productivity-api/file/chat_upload` multipart（file/from/assistant_id）→ `result.file_id/file_url`；消息内 `{"type":"image","image":[{file_id,image_url,file_name,file_size,order,width,height}]}`；同日两模型端到端识图成功 | adapter 已实现 data: URL 上传与引用，NewAPI 端到端验证 | 2026-09-11 |
| `status:10061` 即账号级限流 | A | 2026-09-11 压测后所有请求（含网页端）429：`{"status":10061,"message":"请求过于频繁，请稍后再试"}` | 冷却等待恢复，不要高频试探；视觉请求未见独立错误码 | 2026-09-11 |
